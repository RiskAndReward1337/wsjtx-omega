...
!  Processes timf2 data from Linrad to find and decode JT65 signals.
      ...
      character*6,  intent(in) :: mygrid, hisgrid
      ...
      integer :: ipass_outer1 = 0
      integer :: istored = 0
      logical :: jt65_candidate ! added with modernization
      logical :: l_accept       ! added with modernization
      logical :: l_nonblank     ! added with modernization
      logical :: l_sh_accept    ! added with modernization
...
! No second decode for JT65?  delete for modern version w3sz
...
!    write(*,*) 'GET_CAND: nqd=', nqd, ' ncand=', ncand
!do k = 1, ncand
!   if (abs(cand(k)%f - 11.31) .lt. 0.2) then
!      write(*,*) 'GET_CAND: k=',k,' f=',cand(k)%f,' snr=',cand(k)%snr, &
!                 ' iflip=',cand(k)%iflip
!   endif
!enddo
...
!if (abs(freq - 11.31d0) .lt. 0.02d0) then
!   write(*,*) 'JT65SMAX: freq=', freq, ' smax=', smax
!endif

            if (mode65 .ne. 0 .or. smax .gt. 1.1 .or. ia .eq. ib) then ! changed during moderization
            ! to prevent missed decodes
...
! >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
! FIX #1: For JT65, ALWAYS treat this as a short-decode candidate
! (Legacy behavior; prevents silent minutes like 18:18)
                  if (mode65 .ne. 0) short(1,i) = syncshort
! <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
! >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
! FIX #2: For JT65, BYPASS shorthand-pair requirement entirely
! Jump directly to acceptance block
                     if (mode65 .ne. 0) then
                        i0 = i
                        goto 9001
                     endif
...
!                       sync2=syncshort                   !### try this ???
  ...
  !  write(*,*) 'fshort is: ', fshort,'fshort0 is: ', fshort0,' ftol is: ',ftol, &
  !  ' syncshort is: ',syncshort,'syncshort0 is: ',syncshort0,'nkm is: ',nkm
                           if (fshort - fshort0 .le. ftol .and. &
                               syncshort .gt. syncshort0 .and. nkm .eq. 2) then 
!                                 write(*,*) 'km before 246 decrement = ', km
                                 if (km > 0) km = km - 1
!                                 write(*,*) 'km after 246 decrement = ', km
                           end if
                           if (fshort - fshort0 .gt. ftol .or. &
                               syncshort .gt. syncshort0) then
                              if (km < 0 .or. km > MAXMSG) then
                                write(*,*) 'ERROR: km out of range before 253 sig write: ', km
                              endif
                              if (km < 0) km = 0
...
!                           sig(km,17)=0
                              ...
                              msg(km) = shmsg0(j)
                              ! ---------------------------------------------------------------
                              ! JT65 shorthand acceptance filter + append‑mode logging
                              ! ---------------------------------------------------------------
                              if (mode65 .ne. 0) then

                                 ! shorthand acceptance rule
                                 l_sh_accept = (syncshort .ge. 30.0)

                                 ! append-mode logging for shorthand decodes
                                 open(unit=99, file='JT65_BETA_LOG.TXT', status='unknown', position='append')
                                 write(99,'("SHORTHAND: ",A," syncshort=",F8.2," freq=",F10.3," ACCEPT=",I1)') &
                                      shmsg0(j), syncshort, fshort, merge(1,0,l_sh_accept)
                                 close(99)

                                 ! reject shorthand decode if not accepted
                                 if (.not. l_sh_accept) then
                                    km = km - 1
                                    cycle   ! skip to next frequency bin
                                 endif
...
!if (abs(freq - 11.31d0) .lt. 0.02d0) then
!   write(*,*) 'JT65BIN: freq=', freq, ' sync1=', sync1, ' thresh=', thresh1, &
!              ' freq0=', freq0, ' nkm=', nkm
!endif
...
!                      write(*,*) 'JT65GATE HIT: freq=', freq, ' sync1=', sync1

!  Keep only the best candidate within ftol.
!  (Am I deleting any good decodes by doing this?)
            !      write(*,*) 'freq is: ', freq,'freq0 is: ', freq0,' sync1 is: ',sync1, &
            !      'sync10 is: ',sync10,'nkm is: ',nkm
                  if (freq - freq0 .le. ftol .and. sync1 .gt. sync10 .and. nkm .eq. 1) then 
                !    write(*,*) 'km before 299 decrement = ', km
                    if (km > 0) km = km - 1
                !    write(*,*) 'km after 299 decrement = ', km
                  end if
  ...
  ! Too many calls to decode1a!
   write(*,*) 'MAP65A STRONG-SIGNAL ABORT:'
   write(*,*) '   ntry = ', ntry
   write(*,*) '   nqd  = ', nqd
   write(*,*) '   qual = ', qual
   write(*,*) '   sync1 = ', sync1
   ...
   write(*,*) 'MAP65A: km = ', km
  write(*,*) 'MAP65A: nz = ', nz
  write(*,*) 'MAP65A: nsiz(1:nz) = ', nsiz(1:nz)
...
!                     write(*,*) 'MAP65A CANDIDATE COMPUTED:',&
!     ' ikHz=', ikHz,&
!     ' idf=', idf,&
!     ' df=', freq + 0.5*(nfa+nfb) - foffset - (ikHz + nfshift)

                     nhist = 0  ! Reset deep-search history for this decode pass
                     ...
                     call timer('decode1a', 1)
                      ! ---------------------------------------------------------------
                      ! JT65 acceptance filter + append‑mode logging
                      ! ---------------------------------------------------------------

                      ! Only apply filter for JT65 (mode65 > 0)
                      if (mode65 .ne. 0) then

                         ! Determine if decoded message is nonblank
                         l_nonblank = .false.
                         if (decoded .ne. '                      ') l_nonblank = .true.

                         ! JT65 acceptance rule
                         l_accept = l_nonblank .and. (nkv .eq. 1) .and. (sync2 .ge. 30.0)

                         ! Append‑mode logging for beta testers
                         open(unit=99, file='JT65_BETA_LOG.TXT', status='unknown', position='append')
                         write(99,'("DECODE1A: ",A," sync1=",F6.2," sync2=",F8.2," nkv=",I2," km=",I3, &
                                   " snr2=",F8.2," freq=",F10.3," idf=",I6," df=",F10.6," ACCEPT=",I1)') &
                              decoded, sync1, sync2, nkv, km, snr2, freq, idf, &
                              freq + 0.5*(nfa+nfb) - foffset - (ikHz + nfshift), &
                              merge(1,0,l_accept)
                         close(99)

                         ! If NOT accepted, skip storing this decode entirely
                         if (.not. l_accept) cycle   ! Skip to next candidate
...
! The case sync1=2.0 is just to make sure decode1a is called and bigfft done.
                     ...
                     if (mode65 .ne. 0 .and. sync1 .ne. 2.000000) then
                        istored = istored + 1                        
                        if (km < 0 .or. km > MAXMSG) then
                          write(*,*) 'ERROR: km out of range before 334 sig write: ', km
                        endif
                        if (km < 0) km = 0
                        if (km .lt. MAXMSG) km = km + 1
                        sig(km, 1) = nfile
...
!      write(*,*) 'MAP65A: after trimlist:  km = ', km
!      write(*,*) 'MAP65A: after trimlist:  nz = ', nz
!      write(*,*) 'MAP65A: after trimlist:  nsiz(1:nz) = ', nsiz(1:nz)
...
!write(*,*) 'MAP65A: about to decode candidate i=', i, &
!           ' freq=', sig(i,3), ' sync=', sig(i,4)
