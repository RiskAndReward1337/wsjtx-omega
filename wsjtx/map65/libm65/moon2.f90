...
contains
  subroutine moon2(y,m,Day,UT,lon,lat,RA,Dec,topRA,topDec,LST,HA,Az,El,dist)
    ...
    real*8, intent(in) :: UT                           !UTC in hours
    real*8, intent(out) :: RA,Dec                       !RA and Dec of moon
    real*8, intent(in) :: lat,lon                      !Station coordinates on earth
    real*8, intent(out) :: topRA,topDec                 !Topocentric coordinates of Moon
    real*8, intent(out) :: LST,HA
    real*8, intent(out) :: Az,El
