...
namespace
{
  ...
  {
    auto has_73_token = [](QStringList const& parts) {
      for (auto part : parts) {
        part.remove (QChar {'<'});
        part.remove (QChar {'>'});
        part.remove (QChar {';'});
        if (part == "73" || part == "RR73") return true;
      }
      return false;
    };

    return type >= 0
      && (((type < 6 || 7 == type) && has_73_token (msg_parts))
  ...
  m_useDarkStyle {false}
{
  ...
  // Open the Omega auto-mode debug log file (append mode, UTF-8)
  {
    QString logPath = m_config.writeable_data_dir().absoluteFilePath("omega_auto_debug.log");
    m_autoDebugFile.setFileName(logPath);
    if (m_autoDebugFile.open(QIODevice::WriteOnly | QIODevice::Append | QIODevice::Text)) {
      m_autoDebugStream.setDevice(&m_autoDebugFile);
      m_autoDebugStream.setCodec("UTF-8");
      autoLog("=== WSJTX Omega started ===");
    }
  }
  ...
  connect (ui->decodedTextBrowser, &DisplayText::ignoreCallsign, this, &MainWindow::ignoreAutoStation);
  connect (ui->decodedTextBrowser2, &DisplayText::ignoreCallsign, this, &MainWindow::ignoreAutoStation);
...
// m_settings->setValue("ShMsgs",m_bShMsgs);
  ...
  m_settings->setValue("filter_ignore_cqdx_usa", ui->cb_ignore_cqdx_usa->isChecked());
  m_settings->setValue("filter_ignore_cq_state", ui->cb_ignore_cq_state->isChecked());
  m_settings->setValue("filter_my_state",  ui->le_myState->text());
  m_settings->setValue("autoIgnoreDuration", ui->sb_ignoreDuration->value());
  m_settings->setValue("autoIgnoreIndefinite", ui->cb_ignoreIndefinite->isChecked());
  m_settings->setValue("manualIgnoreList", ui->pte_IgnoredStations->toPlainText());
  // POTA worked-today list: save with the UTC date so we can validate on restore
  m_settings->setValue("potaWorkedDate", m_potaLastDate);
  m_settings->setValue("potaWorkedList", QStringList(m_potaWorkedToday.values()));
...
// m_bShMsgs=m_settings->value("ShMsgs",false).toBool();
  ...
  //ft8md
  ui->actionUse_multithreaded_FT8_decoder->setChecked(m_settings->value("MultithreadedFT8decoder", true).toBool());
  ...
  m_ft8DecoderStart=m_settings->value("FT8DecoderStart",1).toInt();
  ...
  ui->cb_ignore_cqdx_usa->setChecked(m_settings->value("filter_ignore_cqdx_usa", false).toBool());
  ui->cb_ignore_cq_state->setChecked(m_settings->value("filter_ignore_cq_state", false).toBool());
  ui->le_myState->setText(m_settings->value("filter_my_state", "").toString());
  ui->sb_ignoreDuration->setValue(m_settings->value("autoIgnoreDuration", 30).toInt());
  ui->cb_ignoreIndefinite->setChecked(m_settings->value("autoIgnoreIndefinite", false).toBool());
  ui->pte_IgnoredStations->setPlainText(m_settings->value("manualIgnoreList", "").toString());
  // Restore POTA worked-today list only if it belongs to today's UTC date
  {
    QString today = QDateTime::currentDateTimeUtc().date().toString("yyyy-MM-dd");
    QString savedDate = m_settings->value("potaWorkedDate", "").toString();
    if (savedDate == today) {
        QStringList savedList = m_settings->value("potaWorkedList", QStringList()).toStringList();
        m_potaWorkedToday = QSet<QString>(savedList.begin(), savedList.end());
  ...
  ui->actionEnable_AP_FT8->setChecked (m_settings->value ("FT8AP", true).toBool());
  ui->actionEnable_AP_JT65->setChecked (m_settings->value ("JT65AP", true).toBool());
...
void MainWindow::auto_tx_mode (bool state)
{
  if (ui->cbAutoCall->isChecked() || ui->cbAutoHunt->isChecked()
      || ui->cbAutoCQ->isChecked() || ui->cbAutoPOTA->isChecked()) {
    autoLog(QString("auto_tx_mode(%1)  g_iptt=%2  ntx=%3  dxCall='%4'")
            .arg(state).arg(g_iptt).arg(m_ntx).arg(ui->dxCallEntry->text()));
  }
...
void MainWindow::readFromStdout()                             //readFromStdout
{
  ...
  while(proc_jt9.canReadLine()) {
    ...
    // Auto CQ / Auto Call: evaluate filters and track busy frequencies
    if (!m_diskData) {
        callsignFiltered(decodedtext0);
        addSlot(decodedtext0.frequencyOffset());
    }
...
//Left (Band activity) window
      if(!bAvgMsg) {
        ...
        // Auto Hunt panel filter: when hunting, only show CQ POTA stations not yet worked today
        bool huntPanelFiltered = false;
        if (ui->cbAutoHunt->isChecked()) {
          QString huntCall, huntGrid;
          decodedtext.deCallAndGrid(huntCall, huntGrid);
        ...
        if (!huntPanelFiltered && (!filtered or m_config.filters_for_Wait_and_Pounce_only()) && (!ui->cb_filtering->isChecked() || !callsignFiltered(decodedtext))) {  // show decodes if not filtered
          QString distance;
          QString deCall;
          QString deGrid;
          decodedtext.deCallAndGrid(deCall,deGrid);
        ...
        // Auto CQ/POTA may evaluate callers while we are in a CQ cycle, but it
        // must not switch targets until the current CQ transmission has fully
        // ended or the generated reply text can be cleared mid-period.
        bool autoCallingCQReady = !(m_auto && m_bCallingCQ)
                                  || (!m_transmitting && g_iptt == 0 && !m_btxok);
        ...
        decodedtext.deCallAndGrid(/*out*/deCall,grid);
        {
          ...
          if (rpt.size ()       // report in message
              ...
              && (t == deCall   // DX station base call is QSO partner
                  ...
                  || !t.size ())                        // not in QSO
              // Auto CQ/POTA: once we have started replying, freeze the stored
              // signal report.  A drifting SNR would otherwise regenerate our
              // Tx message every period, resetting the watchdog each time and
              // causing an infinite loop against stations we cannot work.
              && !(m_QSOProgress > CALLING
                   && (ui->cbAutoCQ->isChecked () || ui->cbAutoPOTA->isChecked ())))
            {
              m_rptRcvd = rpt;
...
void MainWindow::auto_sequence (DecodedText const& message, unsigned start_tolerance, unsigned stop_tolerance)
{
  ...
  if (message_words.size () > 3 && (message.isStandardMessage() || (is_73 or is_OK))) {
    ...
    QString currentTargetBase = Radio::base_callsign (ui->dxCallEntry->text ());
    bool targetReplyingElsewhere = message_words.size () > 3
      && message_words.at (3).contains (currentTargetBase)
      && !message_words.at (2).contains (m_baseCall)
      && !message_words.at (2).contains (m_config.my_callsign ())
      && !message_words.at (2).contains (currentTargetBase)
      && message_words.at (2) != "DE"
      && message_words.at (3) != "DE"
      && !message_words.at (2).contains (QRegularExpression {"^(CQ|QRZ)$"})
      && !message_words.at (3).contains (QRegularExpression {"^(CQ|QRZ)$"});
    if (m_auto
        && (m_transmitting || g_iptt == 1 || m_btxok)
        && (m_QSOProgress >= REPLYING && m_QSOProgress < ROGERS)
        && SpecOp::HOUND != m_specOp && qAbs (ui->TxFreqSpinBox->value () - df) <= int (stop_tolerance)
        && targetReplyingElsewhere) {
      // auto stop to avoid accidental QRM
      if (ui->cbAutoCall->isChecked() || ui->cbAutoHunt->isChecked()) {
        autoLog(QString("QRM_STOP: target %1 responded to someone else, aborting").arg(ui->dxCallEntry->text()));
        // Target responded to someone else — abort TX, clear state, let ZProcess pick next target.
        // Must call auto_tx_mode(false) so ZProcess sees !autoButton->isChecked() and
        // is free to initiate the next call.  Set m_QSOProgress=CALLING before clearDX
        // so clearDX skips its own auto_tx_mode(false) call (we do it explicitly below).
        addAutoIgnoredStation(ui->dxCallEntry->text(), "target called another station");
        m_btxok = false;
        m_QSOProgress = CALLING;
        clearDX();
        auto_tx_mode(false);
...
// Don't allow Fox mode on WSPR frequencies.
    ...
    if (m_bTxTime && g_iptt == 0 && !m_tune
        && autoCallerQueueEnabled()
        && (ui->cbAutoCQ->isChecked() || ui->cbAutoPOTA->isChecked())
        && (m_ntx == 6 || ui->txrb6->isChecked())) {
      QString const currentDx = ui->dxCallEntry->text().trimmed();
      QStringList const currentDxVariants = auto_call_variants(currentDx);
      bool const staleRecentTarget = !currentDx.isEmpty()
          && (auto_call_variants_overlap(currentDxVariants, m_lastCall)
              || auto_call_variants_overlap(currentDxVariants, m_lastloggedcall)
              || autoQueueCallerShouldSkip(currentDx, ui->dxGridEntry->text().trimmed(), "pre-CQ stale target"));
      if (staleRecentTarget) {
        autoLog(QString("AUTO_QUEUE: clearing stale CQ target '%1' before queue/CQ").arg(currentDx));
        stopWRTimer.stop();
        m_QSOProgress = CALLING;
      ...
      if (ui->dxCallEntry->text().trimmed().isEmpty()
          ...
          && workNextQueuedAutoCaller("pre-CQ queued caller")) {
        autoLog("AUTO_QUEUE: replaced CQ with queued caller before TX");
      }
    }

    if (m_bTxTime && g_iptt == 0 && !m_tune) {
      prepareAutoPotaDualHandoff("pre-TX");
    }
...
//    if(!m_bTxTime and !m_tune and m_mode!="FT4" and m_mode!="FT2") m_btxok=false;       //Time to stop transmitting
    if(!m_bTxTime and !m_tune) {
      if (m_btxok && g_iptt == 1
          && (ui->cbAutoCall->isChecked() || ui->cbAutoHunt->isChecked())) {
        autoLog(QString("TX_KILL: m_bTxTime=false while transmitting  ntx=%1  dxCall='%2'")
                .arg(m_ntx).arg(ui->dxCallEntry->text()));
      }
...
//----------------------------------------------------------------------
...

    {
      ...
      bool const autoDualHandoffTx = isPreparedAutoPotaDualHandoffTx();
      if (autoDualHandoffTx) {
        // genft8_ may canonicalize packed messages. The transmitted bits were
        // generated from tx4, so keep our prepared text for display/log routing.
        m_currentMessage = m_autoDualMessage;
      }
    ...
    bool const autoDualHandoffTx = isPreparedAutoPotaDualHandoffTx();
    bool const autoDualCommitted = autoDualHandoffTx && commitAutoPotaDualHandoffTx();
    if (autoDualHandoffTx) {
      is_73 = true;
    }
    QString active73Target = clean_auto_call_token(ui->dxCallEntry->text());
    if (active73Target.isEmpty()) active73Target = clean_auto_call_token(m_hisCall);
    QStringList const active73TargetVariants = auto_call_variants(active73Target);
    bool const currentAutoTargetNeedsLog = (ui->cbAutoCQ->isChecked() || ui->cbAutoPOTA->isChecked())
      && is_73
      && !autoDualHandoffTx
      && !autoDualCommitted
      && m_QSOProgress >= ROGERS
      && !active73Target.isEmpty()
      && !auto_call_variants_overlap(active73TargetVariants, m_lastloggedcall)
      && !auto_call_variants_overlap(active73TargetVariants, m_lastCall);
    m_sentFirst73 = !autoDualCommitted && (autoDualHandoffTx
      || currentAutoTargetNeedsLog
      || (is_73
          && !message_is_73 (m_lastMessageType, m_lastMessageSent.split (' ', SkipEmptyParts))));
    if (currentAutoTargetNeedsLog
        && message_is_73 (m_lastMessageType, m_lastMessageSent.split (' ', SkipEmptyParts))) {
      autoLog(QString("73_DETECT: logging first signoff for current target '%1' after previous 73 message")
              .arg(active73Target));
    }
    ...
    if(is_73 and (m_config.disable_TX_on_73() or b)) {
      ...
      if(m_nextCall!="") {
        ...
        if(m_config.repeat_Tx() && (m_mode=="MSK144" or m_mode=="Q65")) {
          ...
          // Auto modes self-manage TX state.  Do not let an unrelated decoded 73/RR73
          // kill Auto CQ/POTA/Call/Hunt in the middle of a QSO or while waiting to
          // resume the next cycle.
          bool autoModeActive = ui->cbAutoCQ->isChecked() || ui->cbAutoPOTA->isChecked()
                                || ui->cbAutoCall->isChecked() || ui->cbAutoHunt->isChecked();
          if (!autoModeActive) {
            auto_tx_mode (false);
          } else {
            autoLog("73_DETECT: is_73 detected, NOT killing TX (auto mode manages it)");
          }
          if(b && !autoModeActive) {
            m_ntx=6;
            ui->txrb6->setChecked(true);
...
//----------------------------------------------------------------------
    ...
    if (m_mode != "FST4W" && m_mode != "WSPR" && m_mode!="Echo")
      {
        ...
        if (m_config.TX_messages () && !m_tune && SpecOp::FOX!=m_specOp)
          {
            ui->decodedTextBrowser2->displayTransmittedText(m_currentMessage.trimmed(),
  ...
  if(!m_btxok && m_btxok0 && g_iptt==1) {
    if (ui->cbAutoCall->isChecked() || ui->cbAutoHunt->isChecked()
        || ui->cbAutoCQ->isChecked() || ui->cbAutoPOTA->isChecked()) {
      autoLog(QString("STOP_TX: btxok went false while g_iptt=1  ntx=%1  QSOProgress=%2")
              .arg(m_ntx).arg(m_QSOProgress));
    }
    stopTx();
  ...
  if(nsec != m_sec0) {
    ...
    if(m_mode!="Echo") {
      if(m_monitoring or m_transmitting) {
        ...
        // Auto CQ / Auto Call: fire state machine at the start of each period
        if (isec == 0) ZProcess();
      } else {
...
void MainWindow::processMessage (DecodedText const& message, Qt::KeyboardModifiers modifiers)
{
  ...
  if (auto_cq_pota_only) {
    QString currentTarget = ui->dxCallEntry->text().trimmed();
    QStringList hisVariants = auto_call_variants(hiscall);
    bool messageForMe = message_words.contains(m_baseCall) || message_words.contains(m_config.my_callsign());
    bool repeatFromJustLogged = !hiscall.isEmpty()
        && (auto_call_variants_overlap(hisVariants, m_lastloggedcall)
            || auto_call_variants_overlap(hisVariants, m_lastCall));
    if (!m_bDoubleClicked && repeatFromJustLogged && messageForMe && !is_73) {
      if (!m_autoQueuedCallers.isEmpty() || m_transmitting || g_iptt == 1 || m_btxok) {
        autoLog(QString("AUTO_REPEAT_RR73: heard repeat report from just-logged '%1', not retargeting while busy/queued")
                .arg(hiscall));
        return;
      }

      autoLog(QString("AUTO_REPEAT_RR73: resending one RR73 to just-logged '%1'").arg(hiscall));
      stopWRTimer.stop();
      m_autoTailWindowActive = false;
      m_sentFirst73 = false;
      ui->dxCallEntry->setText(hiscall);
      ...
      m_hisCall = hiscall;
      m_hisGrid = hisgrid;
      genStdMsgs(message.report());
      m_ntx = 4;
      ui->txrb4->setChecked(true);
      m_QSOProgress = CALLING; // send RR73 without logging the same QSO again
      auto_tx_mode(true);
      QTimer::singleShot(int(m_TRperiod * 1200), [=] {
        QString current = ui->dxCallEntry->text().trimmed();
        if (auto_call_variants_overlap(auto_call_variants(current), hiscall)
            && (ui->cbAutoCQ->isChecked() || ui->cbAutoPOTA->isChecked())) {
          m_QSOProgress = CALLING;
          clearDX();
          auto_tx_mode(true);
    ...
    if (!m_bDoubleClicked && lockedToOtherStation && messageForMe && !is_73) {
      if (autoCallerQueueEnabled()) {
        ...
        m_tailenderRpt = message.report();
        m_tailenderTxFirst = m_txFirst;
        m_tailenderGrid = hisgrid;
...
//      w34=w.at(nw-2);
    ...
    bool autoModeActive = ui->cbAutoCQ->isChecked() || ui->cbAutoPOTA->isChecked()
                          || ui->cbAutoCall->isChecked() || ui->cbAutoHunt->isChecked();
    bool freeText73OnCurrentQso = m_QSOProgress >= ROGERS
                                  && !ui->dxCallEntry->text().trimmed().isEmpty()
                                  && (qAbs(ui->RxFreqSpinBox->value() - frequency) <= ui->sbFtol->value()
                                      || qAbs(ui->TxFreqSpinBox->value() - frequency) <= ui->sbFtol->value());
    if (autoModeActive && freeText73OnCurrentQso) {
      autoLog(QString("FREE73: treating non-standard 73 from '%1' as QSO complete")
  ...
  if ((!freeze_auto_cq_report)
      && (!m_bSentReport || base_call != qso_partner_base_call || m_mode == "FT8" || m_mode == "FT4" || m_mode == "FT2"
...
// Don't genStdMsgs if we're already sending 73, or a "TU; " msg is queued.
  ...
  // Auto CQ/POTA: once we have started replying (QSOProgress > CALLING), freeze tx message
  // generation.  Each decode of a repeat-calling station would otherwise update rptSpinBox
...
{
  // Auto POTA mode: force CQ type to "CQ POTA" so tx6 reads "CQ POTA CALLSIGN GRID"
  if (ui->cbAutoPOTA->isChecked()) m_CQtype = "CQ POTA";
...
void MainWindow::clearDX ()
{
  ...
  if (m_QSOProgress != CALLING) {
    ...
    autoLog(QString("clearDX: QSOProgress=CALLING, keeping TX state  g_iptt=%1  m_btxok=%2").arg(g_iptt).arg(m_btxok));
  }
  ui->dxCallEntry->clear ();
  ...
  m_autoTailWindowActive = false;
  genStdMsgs (QString {});
  // A DX reset must always abandon the current QSO partner.  Auto-mode logging
  // paths rely on clearDX() clearing this state before the next CQ/reply cycle.
  m_hisCall = "";
  m_hisCall0 = "";
  if (m_mode=="FT8" and SpecOp::HOUND == m_specOp) {
    m_ntx=1;
  ...
  m_QSOProgress = CALLING;
  // Preserve the just-worked marker while auto modes are active so we do not
  // immediately re-target the station we just logged.
  if (!(ui->cbAutoCall->isChecked() || ui->cbAutoHunt->isChecked()
        || ui->cbAutoCQ->isChecked() || ui->cbAutoPOTA->isChecked())) {
    m_lastCall.clear();
  }
...
void MainWindow::cease_auto_Tx_after_QSO ()
{
  if (SpecOp::FOX != m_specOp
      && ui->cbAutoSeq->isVisible () && ui->cbAutoSeq->isEnabled () && ui->cbAutoSeq->isChecked ())
    {
      // Auto modes self-manage their TX state via acceptQSO() — never kill Enable TX here,
      // or the mode silently stops and the button goes yellow.
      if (ui->cbAutoCQ->isChecked() || ui->cbAutoPOTA->isChecked() ||
          ui->cbAutoCall->isChecked() || ui->cbAutoHunt->isChecked())
        return;
      auto_tx_mode (false);
...
void MainWindow::on_logQSOButton_clicked()                 //Log QSO button
{
  ...
  QString const currentTx = m_currentMessage.trimmed().toUpper();
  QString const dualCompletedBase = Radio::base_callsign(m_autoDualCompletedCall).trimmed().toUpper();
  bool const autoDualLog = m_autoDualLogPending
      && !m_autoDualCompletedCall.isEmpty()
      && currentTx.contains("RR73;")
      && !dualCompletedBase.isEmpty()
      && currentTx.contains(dualCompletedBase);
  if (autoDualLog) {
    autoLog(QString("AUTO_DUAL: logging completed handoff call='%1' next='%2' tx='%3'")
            .arg(m_autoDualCompletedCall).arg(m_autoDualNextCall).arg(m_autoDualMessage));
    m_hisCall = m_autoDualCompletedCall;
    if (!m_autoDualCompletedGrid.isEmpty()) {
      m_hisGrid = m_autoDualCompletedGrid;
      ui->dxGridEntry->setText(m_autoDualCompletedGrid);
    }
    ui->dxCallEntry->setText(m_autoDualCompletedCall);
    if (!m_autoDualRptSent.isEmpty()) m_rptSent = m_autoDualRptSent;
    if (!m_autoDualRptRcvd.isEmpty()) m_rptRcvd = m_autoDualRptRcvd;
    if (!m_autoDualXSent.isEmpty()) m_xSent = m_autoDualXSent;
    if (!m_autoDualXRcvd.isEmpty()) m_xRcvd = m_autoDualXRcvd;
    if (m_autoDualQSOOn.isValid()) m_dateTimeQSOOn = m_autoDualQSOOn;
  ...
  // Capture m_hisCall NOW before initLogQSO — autoLog causes initLogQSO to call accept()
  // synchronously, which emits acceptQSO -> clearDX() -> m_hisCall = "".  Any logic that
  // needs the QSO partner's callsign must run before or use this captured value.
  QString completedCall = m_hisCall;
  QString completedBand = m_currentBand;
  ...
  removeQueuedAutoCallerVariants(completedCallVariants, "just logged");
  ...
  // POTA Hunt: record this station+band as worked today (UTC) so we don't call them again
  if (ui->cbAutoHunt->isChecked() && !completedCall.isEmpty()) {
      QString today = QDateTime::currentDateTimeUtc().date().toString("yyyy-MM-dd");
      if (m_potaLastDate != today) {
  ...
  // Auto Log: record the QSO in the floating terminal window
  if (m_autoLogText && !completedCall.isEmpty() &&
      (ui->cbAutoCQ->isChecked() || ui->cbAutoCall->isChecked() ||
       ui->cbAutoHunt->isChecked() || ui->cbAutoPOTA->isChecked())) {
      QString modeTag = ui->cbAutoHunt->isChecked()  ? "Auto Hunt"
                      : ui->cbAutoPOTA->isChecked()  ? "Auto POTA"
                      : ui->cbAutoCall->isChecked()  ? "Auto Call"
                                                     : "Auto CQ";
      QString utc = QDateTime::currentDateTimeUtc().toString("HH:mm:ss");
      auto lj = [](const QString& s, int w) { return s.leftJustified(w); };
      QString line = lj(utc, 8)           + "  " +
                     lj(completedCall, 12) + "  " +
                     lj(completedBand, 6) + "  " +
                     lj(m_mode, 6)        + "  " +
                     lj(m_rptSent, 4)     + "  " +
                     lj(m_rptRcvd, 4)     + "  " +
                     lj(m_hisGrid.left(6), 6) + "  " +
                     modeTag;
      m_autoLogText->append(line);
  }
                            ...
                            , QString const& freqRx, QByteArray const& ADIF)
{
  ...
  if (ui->cbAutoCQ->isChecked() || ui->cbAutoPOTA->isChecked()) {
    autoLog(QString("acceptQSO: QSO complete (AutoCQ/POTA), resuming CQ  call=%1").arg(call));
    // acceptQSO fires at the start of the final RR73/73 period. Let that finish
    // before clearing the QSO and resuming with CQ or a queued tail-ender.
    m_autoTailWindowActive = true;
    QTimer::singleShot(int(m_TRperiod * 1200), [=] {
        if (!(ui->cbAutoCQ->isChecked() || ui->cbAutoPOTA->isChecked())) {
            m_autoTailWindowActive = false;
            autoLog("acceptQSO: CQ/POTA resume timer expired after auto mode was turned off");
            return;
        }
        QString currentDx = ui->dxCallEntry->text().trimmed();
        if (!currentDx.isEmpty() && currentDx != call) {
            m_autoTailWindowActive = false;
            autoLog(QString("acceptQSO: CQ/POTA resume timer sees active target '%1', leaving it alone")
                    .arg(currentDx));
            return;
        }
        m_QSOProgress = CALLING;
        clearDX();
        if (workNextQueuedAutoCaller("acceptQSO post-signoff")) {
            // Queue helper selected the next station and re-enabled TX.
        } else if (!autoCallerQueueEnabled()
                   && !m_tailenderCall.isEmpty() && m_tailenderCall != call) {
            autoLog(QString("acceptQSO: resuming with queued caller '%1' instead of CQ").arg(m_tailenderCall));
            m_nextCall = m_tailenderCall;
            m_nextGrid = m_tailenderGrid;
            m_nextRpt = m_tailenderRpt;
            ui->rptSpinBox->setValue(m_tailenderRpt.toInt());
            ui->txFirstCheckBox->setChecked(m_tailenderTxFirst);
            ui->RxFreqSpinBox->setValue(m_tailenderFreq);
            if (!ui->cbHoldTxFreq->isChecked()) ui->TxFreqSpinBox->setValue(m_tailenderFreq);
            useNextCall();
            on_txb2_clicked();
            m_tailenderCall.clear();
            m_tailenderGrid.clear();
    ...
    auto finishAutoCallHuntQSO = [=] {
        if (ui->cbAutoCall->isChecked() || ui->cbAutoHunt->isChecked()) {
            ...
            if (ui->dxCallEntry->text().isEmpty()) {
                if (!workNextQueuedAutoCaller("AutoCall/Hunt cleanup")) {
  ...
  if (triggered)
    {
      ...
      // Auto CQ / Auto POTA / Auto Call / Auto Hunt: watchdog fired because the
      // other station can't hear us.  Abandon this QSO attempt and resume the
      // active auto mode rather than staying stuck with Enable TX dead.
      if (ui->cbAutoCQ->isChecked () || ui->cbAutoPOTA->isChecked ()
          || ui->cbAutoCall->isChecked () || ui->cbAutoHunt->isChecked ()) {
        ...
        if ((ui->cbAutoCQ->isChecked () || ui->cbAutoPOTA->isChecked ())
          ...
          addAutoIgnoredStation(stalledTarget, "AutoCQ/POTA watchdog timeout");
        } else if ((ui->cbAutoCall->isChecked () || ui->cbAutoHunt->isChecked ())
                   && !m_autoCallTarget.isEmpty ()) {
          if (ui->cbAutoHunt->isChecked ()) {
            addAutoIgnoredStation(m_autoCallTarget, "Auto Hunt watchdog timeout");
          } else {
            // Auto Call: use the configured ignore timer
            int dur = ui->sb_ignoreDuration->value ();
            bool indefinite = ui->cb_ignoreIndefinite->isChecked ();
            if (indefinite || dur > 0) {
              QDateTime expiry = indefinite ? QDateTime ()
                                            : QDateTime::currentDateTimeUtc ().addSecs (dur * 60);
              m_autoIgnored.insert (m_autoCallTarget, expiry);
              refreshAutoIgnoredList ();
            }
          }
        }
        clearDX ();
        QTimer::singleShot (500, [=] {
          tx_watchdog (false);   // clear the red watchdog state
          // Auto CQ / Auto POTA need Enable TX on so they can immediately resume
          // sending CQ.  Auto Call / Auto Hunt must NOT re-enable TX here because
          // ZProcess only initiates calls when autoButton is unchecked; re-enabling
          // TX would make ZProcess skip all decoded stations and leave the mode
          // stuck (transmitting nothing, responding to nobody).  ZProcess naturally
          // re-enables TX when it picks the next target station.
          if (ui->cbAutoCQ->isChecked () || ui->cbAutoPOTA->isChecked ()) {
            auto_tx_mode (true);
          }
        });
  ...
  if (m_hisCall!="") {
    ...
    if(ignoreFile.open(QIODevice::Text | QIODevice::Append)) {
      ...
      out << ignoreEntry <<
                   ...
                   ;
      ignoreFile.close();
      removeQueuedAutoCallerVariants(auto_call_variants(m_hisCall), "legacy ignore list");
...
void MainWindow::addAutoIgnoredStation(QString const& callsign, QString const& reason)
{
  auto normalized = callsign.trimmed().toUpper();
  if (normalized.isEmpty()) return;

  int dur = ui->sb_ignoreDuration->value();
  bool indefinite = ui->cb_ignoreIndefinite->isChecked();
  if (!indefinite && dur <= 0) {
    autoLog(QString("AUTO_IGNORE: skipped '%1' because ignore duration is disabled").arg(normalized));
    return;
  }

  QDateTime expiry = indefinite ? QDateTime {}
                              : QDateTime::currentDateTimeUtc().addSecs(dur * 60);
  m_autoIgnored.insert(normalized, expiry);
  removeQueuedAutoCallerVariants(auto_call_variants(normalized), "auto-ignore");
  refreshAutoIgnoredList();

  QString detail = reason.isEmpty() ? QString {} : QString("  reason=%1").arg(reason);
  if (indefinite) {
    autoLog(QString("AUTO_IGNORE: added '%1' indefinitely%2").arg(normalized, detail));
    showStatusMessage(tr("Ignoring %1 indefinitely").arg(normalized));
  } else {
    autoLog(QString("AUTO_IGNORE: added '%1' for %2 minutes%3").arg(normalized).arg(dur).arg(detail));
    showStatusMessage(tr("Ignoring %1 for %2 minute(s)").arg(normalized).arg(dur));
  }
}

void MainWindow::ignoreAutoStation(QString const& callsign)
{
  addAutoIgnoredStation(callsign, "manual decode ignore");
}
...
{
    autoLog(QString("AUTO_CQ toggled %1").arg(b ? "ON" : "OFF"));
    if (b) {
        ui->cbAutoCall->setChecked(false);
        ui->cbAutoCall->setEnabled(false);
        ui->cbAutoPOTA->setChecked(false);
        ui->cbAutoPOTA->setEnabled(false);
        ui->cbAutoSeq->setChecked(true);
        ui->txrb6->setChecked(true);
        resetAutoSwitch();
        if (!m_autoModeSwitch) clearDX();
        // Auto CQ: always use RR73 so each QSO completes in one fewer transmission.
        m_send_RR73 = true;
        genStdMsgs(m_rpt);
    } else {
...
{
    autoLog(QString("AUTO_CALL toggled %1").arg(b ? "ON" : "OFF"));
    if (b) {
        ...
        if (ui->cbAutoHunt->isChecked()) ui->cbAutoHunt->setChecked(false);
    }
...
// ── Omega auto-mode debug log ──────────────────────────────────────────────
void MainWindow::autoLog(QString const& msg)
{
    if (!m_autoDebugFile.isOpen()) return;
    QString ts = QDateTime::currentDateTimeUtc().toString("yyyy-MM-dd hh:mm:ss.zzz");
    m_autoDebugStream << ts << "  " << msg << "\n";
...
void MainWindow::resetAutoSwitch()
{
    ...
    m_autoDualLogPending = false;
    m_autoDualCompletedCall = QString();
    m_autoDualCompletedGrid = QString();
    m_autoDualNextCall = QString();
    m_autoDualNextGrid = QString();
    m_autoDualNextRpt = QString();
    ...
    m_autoDualXRcvd = QString();
    m_autoDualQSOOn = QDateTime {};
    m_tailenderCall = QString();
    m_tailenderGrid = QString();
...
bool MainWindow::autoQueueCallerShouldSkip(QString const& call, QString const& grid, QString const& context)
{
    ...
    if (!plausible_auto_callsign(queuedCall)) return true;

    QStringList variants = auto_call_variants(queuedCall);
    if (auto_call_variants_overlap(variants, m_hisCall)
        || auto_call_variants_overlap(variants, m_lastCall)
        || auto_call_variants_overlap(variants, m_lastloggedcall)) {
        autoLog(QString("AUTO_QUEUE: skip %1 (recent/current QSO)  context=%2").arg(queuedCall, context));
        removeQueuedAutoCallerVariants(variants, "recent/current QSO");
        return true;
    }

    for (auto it = m_autoIgnored.begin(); it != m_autoIgnored.end(); ++it) {
        if (!auto_call_variants_overlap(variants, it.key())) continue;
        QDateTime expiry = it.value();
        if (!expiry.isValid() || QDateTime::currentDateTimeUtc() < expiry) {
            autoLog(QString("AUTO_QUEUE: skip %1 (auto-ignored as %2)  context=%3")
                    .arg(queuedCall).arg(it.key()).arg(context));
            removeQueuedAutoCallerVariants(variants, "auto-ignored");
            return true;
        }
    }

    QStringList ignoredText = ui->pte_IgnoredStations->toPlainText()
                                  .split(QRegExp("[\r\n]"), SkipEmptyParts);
    for (QString ignoredCall : ignoredText) {
        ignoredCall = ignoredCall.trimmed().toUpper();
        if (ignoredCall.isEmpty()) continue;
        if (auto_call_variants_overlap(variants, ignoredCall)) {
            autoLog(QString("AUTO_QUEUE: skip %1 (ignored station list)  context=%2")
                    .arg(queuedCall, context));
            removeQueuedAutoCallerVariants(variants, "ignored station list");
            return true;
        }
    }
    for (auto const& variant : variants) {
        if (ignoreList.contains(variant + ",")) {
            autoLog(QString("AUTO_QUEUE: skip %1 (legacy ignore list)  context=%2")
                    .arg(queuedCall, context));
            removeQueuedAutoCallerVariants(variants, "legacy ignore list");
            return true;
        }
    }
    ...
    if (ui->cbAutoHunt->isChecked()) {
        ...
        QString workedKey = pota_worked_key_match(m_potaWorkedToday, variants, m_currentBand);
        if (!workedKey.isEmpty()) {
            autoLog(QString("AUTO_QUEUE: skip %1 (POTA worked today as %2)  context=%3")
                    .arg(queuedCall, workedKey, context));
            removeQueuedAutoCallerVariants(variants, "POTA worked today");
            return true;
        }
    }

    if (ui->cb_filtering->isChecked()
        && !ui->cbAutoHunt->isChecked()
        && (ui->cb_callB4->isChecked()        || ui->cb_callB4onBand->isChecked()      ||
            ui->cb_countryB4->isChecked()     || ui->cb_countryB4onBand->isChecked()   ||
            ui->cb_gridB4->isChecked()        || ui->cb_gridB4onBand->isChecked()      ||
            ui->cb_continentB4->isChecked()   || ui->cb_continentB4onBand->isChecked() ||
            ui->cb_CQZoneB4->isChecked()      || ui->cb_CQZoneB4onBand->isChecked()    ||
            ui->cb_ITUZoneB4->isChecked()     || ui->cb_ITUZoneB4onBand->isChecked())) {
        bool callB4 = false;
        bool callB4onBand = false;
        bool countryB4 = false;
        bool countryB4onBand = false;
        bool gridB4 = false;
        bool gridB4onBand = false;
        ...
        auto const& looked_up = m_logBook.countries()->lookup(queuedCall);
        m_logBook.match(queuedCall, m_mode, grid, looked_up,
        ...
        for (auto const& variantCall : variants) {
            if (variantCall == queuedCall) continue;
            ...
            m_logBook.match(variantCall, m_mode, grid, variantLookedUp,
        ...
        QString skipReason;
        if (ui->cb_callB4->isChecked() && callB4) skipReason = "call worked before";
        else if (ui->cb_callB4onBand->isChecked() && callB4onBand) skipReason = "call worked on band";
        else if (ui->cb_countryB4->isChecked() && countryB4) skipReason = "country worked before";
        else if (ui->cb_countryB4onBand->isChecked() && countryB4onBand) skipReason = "country worked on band";
        else if (ui->cb_gridB4->isChecked() && gridB4) skipReason = "grid worked before";
        else if (ui->cb_gridB4onBand->isChecked() && gridB4onBand) skipReason = "grid worked on band";
        else if (ui->cb_continentB4->isChecked() && continentB4) skipReason = "continent worked before";
        else if (ui->cb_continentB4onBand->isChecked() && continentB4onBand) skipReason = "continent worked on band";
        else if (ui->cb_CQZoneB4->isChecked() && CQZoneB4) skipReason = "CQ zone worked before";
        else if (ui->cb_CQZoneB4onBand->isChecked() && CQZoneB4onBand) skipReason = "CQ zone worked on band";
        else if (ui->cb_ITUZoneB4->isChecked() && ITUZoneB4) skipReason = "ITU zone worked before";
        else if (ui->cb_ITUZoneB4onBand->isChecked() && ITUZoneB4onBand) skipReason = "ITU zone worked on band";

        if (!skipReason.isEmpty()) {
            autoLog(QString("AUTO_QUEUE: skip %1 (%2)  context=%3").arg(queuedCall, skipReason, context));
            removeQueuedAutoCallerVariants(variants, skipReason);
            return true;
        }
    }
                                 ...
                                 int freq, bool txFirst, QString const& reason)
{
    ...
    QStringList variants = auto_call_variants(queuedCall);
    if (autoQueueCallerShouldSkip(queuedCall, grid, reason)) return;

    AutoQueuedCaller queued;
    queued.call = queuedCall;
    queued.grid = grid;
    ...
    while (m_autoQueuedCallers.size() >= 12) {
        auto dropped = m_autoQueuedCallers.dequeue();
        autoLog(QString("AUTO_QUEUE: dropped oldest %1 (queue full)").arg(dropped.call));
    }
    m_autoQueuedCallers.enqueue(queued);
...
bool MainWindow::workNextQueuedAutoCaller(QString const& reason)
{
    ...
    if (m_transmitting || g_iptt == 1) return false;

    // CQ/POTA callers are fallback work. Auto Call/Hunt CQ priority gets first refusal.
    if ((ui->cbAutoCall->isChecked() || ui->cbAutoHunt->isChecked())
    ...
    QString currentDx = ui->dxCallEntry->text().trimmed();
    if (!currentDx.isEmpty()) return false;
    ...
    while (!m_autoQueuedCallers.isEmpty()) {
        ...
        if (!plausible_auto_callsign(queued.call)
            || auto_call_variants_overlap(variants, m_hisCall)
            || auto_call_variants_overlap(variants, m_lastCall)
            || auto_call_variants_overlap(variants, m_lastloggedcall)) {
        ...
        for (auto it = m_autoIgnored.begin(); it != m_autoIgnored.end(); ++it) {
            ...
            if (!expiry.isValid() || now < expiry) {
                ignored = true;
                autoLog(QString("AUTO_QUEUE: skipped %1 (auto-ignored as %2)")
                        .arg(queued.call).arg(it.key()));
                break;
            }
        }
        if (ignored) {
        ...
        autoLog(QString("AUTO_QUEUE: working %1  freq=%2  rpt=%3  grid=%4  reason=%5  remaining=%6")
                .arg(queued.call).arg(queued.freq).arg(queued.report).arg(queued.grid)
                .arg(reason).arg(m_autoQueuedCallers.size()));
        tx_watchdog(false);
        m_nextCall = queued.call;
        m_nextGrid = queued.grid;
        m_nextRpt = queued.report;
        ui->rptSpinBox->setValue(queued.report.toInt());
        ui->txFirstCheckBox->setChecked(queued.txFirst);
        ui->RxFreqSpinBox->setValue(queued.freq);
        if (!ui->cbHoldTxFreq->isChecked()) ui->TxFreqSpinBox->setValue(queued.freq);
        useNextCall();
        on_txb2_clicked();
...
bool MainWindow::isPreparedAutoPotaDualHandoffTx() const
{
    return m_autoDualLogPending
        && m_ntx == 4
        && !m_autoDualMessage.isEmpty()
        && ui->tx4->text().trimmed().compare(m_autoDualMessage, Qt::CaseInsensitive) == 0;
}

bool MainWindow::commitAutoPotaDualHandoffTx()
{
    if (!isPreparedAutoPotaDualHandoffTx()) return false;

    QString const completedCall = m_autoDualCompletedCall;
    QString const completedGrid = m_autoDualCompletedGrid;
    QString const nextCall = m_autoDualNextCall;
    QString const nextGrid = m_autoDualNextGrid;
    QString const nextReport = m_autoDualNextRpt;
    int const nextFreq = m_autoDualNextFreq;
    bool const nextTxFirst = m_autoDualNextTxFirst;

    if (completedCall.isEmpty() || nextCall.isEmpty() || nextReport.isEmpty()) return false;

    autoLog(QString("AUTO_DUAL: committing DXpedition-style handoff completed='%1' next='%2' tx='%3'")
            .arg(completedCall).arg(nextCall).arg(m_autoDualMessage));

    // Log the station receiving RR73 now, like Fox mode does when it builds
    // "old RR73; new <MYCALL> rpt". Do not wait for their courtesy 73.
    m_hisCall = completedCall;
    if (!completedGrid.isEmpty()) {
        m_hisGrid = completedGrid;
        ui->dxGridEntry->setText(completedGrid);
    }
    ui->dxCallEntry->setText(completedCall);
    if (!m_autoDualRptSent.isEmpty()) m_rptSent = m_autoDualRptSent;
    if (!m_autoDualRptRcvd.isEmpty()) m_rptRcvd = m_autoDualRptRcvd;
    if (!m_autoDualXSent.isEmpty()) m_xSent = m_autoDualXSent;
    if (!m_autoDualXRcvd.isEmpty()) m_xRcvd = m_autoDualXRcvd;
    ...
    QStringList const nextVariants = auto_call_variants(nextCall);
    for (int i = 0; i < m_autoQueuedCallers.size(); ++i) {
        if (auto_call_variants_overlap(nextVariants, m_autoQueuedCallers.at(i).call)) {
            m_autoQueuedCallers.removeAt(i);
            break;
        }
    }

    // Promote the reported station to the active QSO before their next decode
    // arrives. We already sent their report in the packed handoff.
    ui->dxCallEntry->setText(nextCall);
    ui->dxGridEntry->setText(nextGrid);
    m_hisCall = nextCall;
    m_hisGrid = nextGrid;
    m_rptSent = nextReport;
    m_rptRcvd.clear();
    m_xSent.clear();
    m_xRcvd.clear();
    m_nextCall.clear();
    m_nextGrid.clear();
    m_nextRpt.clear();
    m_autoCallTarget = nextCall;
    ui->rptSpinBox->setValue(nextReport.toInt());
    ui->txFirstCheckBox->setChecked(nextTxFirst);
    if (nextFreq > 0) {
        ui->RxFreqSpinBox->setValue(nextFreq);
        if (!ui->cbHoldTxFreq->isChecked()) ui->TxFreqSpinBox->setValue(nextFreq);
    }
    set_dateTimeQSO(2);
    genStdMsgs(QString::number(ui->rptSpinBox->value()));
    m_ntx = 2;
...
bool MainWindow::prepareAutoPotaDualHandoff(QString const& reason, bool allowStartingTx)
{
    ...
    bool finalSignoffTx = (m_ntx == 4 || ui->txrb4->isChecked()
                           || m_ntx == 5 || ui->txrb5->isChecked());
    if (!finalSignoffTx) {
        if (reason == "encode-TX") {
            autoLog(QString("AUTO_DUAL: encode skip (not final signoff)  ntx=%1  QSOProgress=%2  queued=%3")
                    .arg(m_ntx).arg(m_QSOProgress).arg(m_autoQueuedCallers.size()));
        }
        return false;
    }

    QString completedCall = clean_auto_call_token(ui->dxCallEntry->text());
    if (completedCall.isEmpty()) completedCall = clean_auto_call_token(m_hisCall);
    QString completedBase = Radio::base_callsign(completedCall).trimmed().toUpper();
    if (!plausible_auto_callsign(completedBase)) return false;
    if (completedBase.length() > 6) return false;

    QString const tx4Text = ui->tx4->text().trimmed().toUpper();
    QString const tx5Text = ui->tx5->currentText().trimmed().toUpper();
    bool canReplaceCurrentSignoff = (m_ntx == 5 || ui->txrb5->isChecked())
        || tx4Text.contains("RR73");
    bool hasFinalSignoff = tx4Text.contains("RR73") || tx5Text.contains("73")
        || m_QSOProgress >= ROGER_REPORT;
    if (!canReplaceCurrentSignoff || !hasFinalSignoff) {
        if (reason == "encode-TX") {
            autoLog(QString("AUTO_DUAL: encode skip (signoff not replaceable)  ntx=%1  tx4='%2'  tx5='%3'  QSOProgress=%4")
                    .arg(m_ntx).arg(tx4Text).arg(tx5Text).arg(m_QSOProgress));
        }
        return false;
    }
    ...
    while (!m_autoQueuedCallers.isEmpty()) {
        ...
        for (auto it = m_autoIgnored.begin(); it != m_autoIgnored.end(); ++it) {
            ...
            if (!expiry.isValid() || now < expiry) {
                ignored = true;
                autoLog(QString("AUTO_DUAL: skipped %1 (auto-ignored as %2)")
                        .arg(queued.call).arg(it.key()));
                break;
            }
        }
        ...
        QString nextBase = Radio::base_callsign(queued.call).trimmed().toUpper();
        ...
        QString report = queued.report.trimmed().toUpper();
        if (report.startsWith("R")) report = report.mid(1);
        bool reportOk = false;
        int const reportValue = report.toInt(&reportOk);
        ...
        if (m_autoDualCompletedGrid.isEmpty()) m_autoDualCompletedGrid = m_hisGrid;
        m_autoDualNextCall = queued.call;
        m_autoDualNextGrid = queued.grid;
        m_autoDualNextRpt = report;
        m_autoDualNextFreq = queued.freq;
        m_autoDualNextTxFirst = queued.txFirst;
        m_autoDualMessage = dualMessage;
        m_autoDualRptSent = m_rptSent;
        m_autoDualRptRcvd = m_rptRcvd;
        m_autoDualXSent = m_xSent;
        m_autoDualXRcvd = m_xRcvd;
        m_autoDualQSOOn = m_dateTimeQSOOn;
        if (ui->tx4->text().trimmed() != dualMessage) {
            msgtype(dualMessage, ui->tx4);
            autoLog(QString("AUTO_DUAL: prepared '%1'  reason=%2  queued=%3")
                    .arg(dualMessage).arg(reason).arg(m_autoQueuedCallers.size()));
        }
        ui->txrb4->setChecked(true);
        m_ntx = 4;
        refreshAutoQueueWindow();
...
// Called once per TX/RX period (at isec == 0) to manage Auto CQ / Auto Call state
void MainWindow::ZProcess()
{
    if (m_transmitting) {
        // Auto CQ / Auto POTA may queue a tail-ender while we are finishing the
        // current QSO.  Preserve that queued caller through the final 73 so
        // acceptQSO() can hand off to them instead of falling back to CQ.
        if (!(ui->cbAutoCQ->isChecked() || ui->cbAutoPOTA->isChecked())) {
            m_priorityCall = QString();
        }
    ...
    // If Auto Call is active and we're on txrb6 (CQ slot), clear DX to start fresh
    ...
    bool priorityAlreadyWorked = auto_call_variants_overlap(priorityCallVariants, m_lastCall)
                                  || auto_call_variants_overlap(priorityCallVariants, m_lastloggedcall);
    ...
    bool startedPriorityCall = false;
    if (!ui->autoButton->isChecked() && autoCallActive
        ...
        on_txb1_clicked();
        auto_tx_mode(true);
        m_autoCallTarget = m_priorityCall;   // track for auto-ignore
        if (m_config.highlight_DXcall())
            ui->decodedTextBrowser->highlight_callsign(m_priorityCall, QColor(255,0,0), QColor(255,255,255), true);
        startedPriorityCall = true;
    } else if (autoCallActive) {
        autoLog(QString("ZProcess: no call  prio='%1'  autoBtn=%2  lastCall='%3'  dxEntry='%4'")
                .arg(m_priorityCall).arg(ui->autoButton->isChecked())
                .arg(m_lastCall).arg(ui->dxCallEntry->text()));
    }

    if (!startedPriorityCall) {
        workNextQueuedAutoCaller("ZProcess");
    }

    // Roll the busy-slot history window (keep last 3 periods)
    int numVectors = 3;
...
{
    if (busySlots.isEmpty()) busySlots.prepend(QVector<int>());
    QVector<int>& slotsV = busySlots[0];
    slotsV.append(freq);
}

// Check whether frequency f is clear (no decode within mode bandwidth)
bool MainWindow::isSlotFree(int f)
{
    // FT4 is ~80 Hz wide, FT8 is ~50 Hz wide; use 100 Hz guard band
    int bw = (m_mode == "FT4") ? 80 : 50;
    for (int x = 0; x < busySlots.size(); x++) {
        for (int occupied : busySlots.at(x)) {
            if (qAbs(occupied - f) < bw) return false;
        }
    }
    return true;
}

// Find a clear TX frequency and tune to it; returns true if a free slot was found
bool MainWindow::setFreeFreq()
{
    if (busySlots.size() < 2) return false;
    // Search down from 1500 Hz
    for (int f = 1500; f >= 500; f -= 10) {
        if (isSlotFree(f)) { ui->TxFreqSpinBox->setValue(f); return true; }
    }
    // Search up from 1500 Hz
    for (int f = 1510; f <= 2000; f += 10) {
        if (isSlotFree(f)) { ui->TxFreqSpinBox->setValue(f); return true; }
    }
    return false;
}
...
bool MainWindow::callsignFiltered(DecodedText dt)
{
    ...
    if (message_words.size() > 3) {
        addressedStation = normalizedWord(message_words.at(2));
        callingStation = normalizedWord(message_words.at(3));
    }
    if (rawCallTokens.size() >= 1 && addressedStation.isEmpty()) {
        addressedStation = rawCallTokens.at(0);
    }
    if (rawCallTokens.size() >= 2 && callingStation.isEmpty()) {
        callingStation = rawCallTokens.at(1);
    }
    QString candidateCall = dxCall;
    ...
    if (!plausible_auto_callsign(candidateCall)
        ...
        bool messageLooksCallLike = dt.messageText().contains(QRegularExpression{"[A-Za-z]"});
        if ((ui->cbAutoCQ->isChecked() || ui->cbAutoPOTA->isChecked())
                && postSignoffWindow && messageLooksCallLike) {
            autoLog(QString("TAILENDER: unresolved caller  raw='%1'  words='%2'  dxCall='%3'")
                    .arg(dt.messageText())
                    .arg(message_words.join("|"))
                    .arg(dxCall));
        }
        return true;
    }
    // Suppress likely-false /R decodes
    if (candidateCall.endsWith("/R")) return true;
    ...
    bool decoded_txFirst = (nmod != 0);

    bool is_73 = (message_words.size() >= 5 &&
                  (message_words.contains("73") || message_words.contains("RR73")));
    bool is_CQ = message_words.filter(QRegularExpression{"^(CQ|CQ\\s\\w+)$"}).size() > 0;

    // Auto CQ/POTA pileups are callers answering us, not CQ candidates. Capture
    // them before the normal CQ/new-grid/filter path can discard them.
    if ((ui->cbAutoCQ->isChecked() || ui->cbAutoPOTA->isChecked())
        && !ui->cbAutoCall->isChecked() && !ui->cbAutoHunt->isChecked()
        && autoCallerQueueEnabled()) {
        ...
        bool is_for_me = addressedStation == m_baseCall
                         ...
                         || message_words.contains(m_baseCall)
                         || message_words.contains(m_config.my_callsign());
        bool active_or_transitioning = !currentTarget.isEmpty()
                                       || m_autoTailWindowActive
                                       || m_transmitting
                                       || g_iptt == 1
                                       || m_btxok
                                       || m_QSOProgress > CALLING;
        QStringList queuedVariants = auto_call_variants(queuedCall);
        bool different_from_active = currentTarget.isEmpty()
                                     || !auto_call_variants_overlap(queuedVariants, currentTarget);
        if (active_or_transitioning
            && is_for_me
            && !is_73
            && different_from_active
            && !auto_call_variants_overlap(queuedVariants, m_hisCall)
            && !auto_call_variants_overlap(queuedVariants, m_lastCall)
            && !auto_call_variants_overlap(queuedVariants, m_lastloggedcall)) {
    ...
    // Never filter the current QSO partner while the exchange is still live.
    // Do not exempt m_lastCall here: Auto CQ/POTA intentionally preserves it so a
    ...
    if (!ui->cb_filtering->isChecked()) {
        // Filtering disabled: pass everything (but still track CQ priority below)
        passedFilters = true;
    } else {
        // LOTW-only filter
        if (ui->cb_f_LOTW->isChecked() && !m_config.lotw_users().user(dxCall)) return true;

        // Ignored-stations list
        {
        ...
        if (ui->sbMindB->value() > -30 && dt.report().toInt() < ui->sbMindB->value()) return true;
        ...
        // Continent filter
        {
            ...
            // Hide USA stations calling "CQ DX" — they are seeking non-US contacts
            if (ui->cb_ignore_cqdx_usa->isChecked()
                    && looked_up.entity_name.contains("United States", Qt::CaseInsensitive)
                    && message_words.contains("CQ DX")) return true;
        }

        // State CQ filter: hide stations calling CQ [state] when state != my state
        if (ui->cb_ignore_cq_state->isChecked()) {
            QString myState = ui->le_myState->text().trimmed().toUpper();
            if (!myState.isEmpty() && message_words.size() > 1) {
                QString word1 = message_words.at(1); // e.g. "CQ ND"
                if (word1.startsWith("CQ ")) {
                    QString cqState = word1.mid(3).trimmed().toUpper();
                    ...
                    if (usStates.contains(cqState) && cqState != myState) return true;
                }
            }
        }

        // Worked-before / new-on-band filters
        // Auto Hunt skips these — POTA operators call CQ repeatedly and should be workable
        // on any band regardless of prior QSOs; the per-UTC-day per-band m_potaWorkedToday
        // tracker is the correct dedup mechanism for Hunt mode.
        if (!ui->cbAutoHunt->isChecked() &&
            (ui->cb_callB4->isChecked()        || ui->cb_callB4onBand->isChecked()      ||
            ui->cb_countryB4->isChecked()     || ui->cb_countryB4onBand->isChecked()   ||
            ui->cb_gridB4->isChecked()        || ui->cb_gridB4onBand->isChecked()      ||
    ...
    // Auto CQ / Auto POTA: remember callers during the final RR73/73 stage.
    // With the post-QSO option enabled, also queue callers that arrive while
    // we have already resumed CQ but are still hearing stations answer us.
    if ((ui->cbAutoCQ->isChecked() || ui->cbAutoPOTA->isChecked())
        && !ui->cbAutoCall->isChecked() && !ui->cbAutoHunt->isChecked()) {
        QString tailenderCall = dxCall;
        if (!callingStation.isEmpty()
            && callingStation != m_baseCall
            && callingStation != myBase
            && callingStation != addressedStation) {
            tailenderCall = callingStation;
        }
        ...
        bool post_qso_cq_window = autoCallerQueueEnabled()
                                  && no_active_target
        ...
        bool can_immediately_seize_caller = ready_to_work_now
                                            ...
                                            && !auto_call_variants_overlap(auto_call_variants(tailenderCall), m_lastloggedcall);
        if (can_immediately_seize_caller) {
            m_tailenderCall = tailenderCall;
            m_tailenderFreq = dt.frequencyOffset();
            m_tailenderRpt = dt.report();
            m_tailenderTxFirst = decoded_txFirst;
            m_tailenderGrid = dxGrid;
            autoLog(QString("TAILENDER: seizing %1 immediately instead of restarting CQ")
                    .arg(tailenderCall));
            m_nextCall = m_tailenderCall;
            m_nextGrid = m_tailenderGrid;
            m_nextRpt = m_tailenderRpt;
        ...
        bool can_queue_tailender = (finishing_qso || post_qso_cq_window || active_pota_pileup_queue)
                                   && is_for_me
                                   && !is_73
                                   && different_from_active
                                   && !auto_call_variants_overlap(auto_call_variants(tailenderCall), m_hisCall)
        ...
        if (can_queue_tailender) {
            if (autoCallerQueueEnabled()) {
                QString reason = finishing_qso ? "signoff tailender"
                ...
                m_tailenderTxFirst = decoded_txFirst;
                m_tailenderGrid = dxGrid;
                autoLog(QString("TAILENDER: queued %1  freq=%2  rpt=%3  grid=%4  txFirst=%5")
                        .arg(tailenderCall).arg(m_tailenderFreq).arg(m_tailenderRpt).arg(dxGrid).arg(m_tailenderTxFirst));
            }
        }
    }
    ...
    // Only CQ (and optionally 73) messages are eligible for Auto Call priority tracking
    if (!is_CQ && !(ui->cbCQonlyIncl73->isChecked() && is_73)) return false;

    // TX-first slot consistency check (avoid calling stations on the wrong slot)
    if (m_TxFirstLock && ui->txFirstCheckBox->isChecked() != decoded_txFirst) return false;

    // Priority call selection for Auto Call
    if (!ui->cbAutoCall->isChecked()) return false;

    bool prio = false;
    if (ui->cb_autoCallPriority->currentIndex() == 2) {
        // Most distant
        if (dxGrid.length() == 4 && dxGrid != "RR73" &&
            !dxGrid.startsWith("R-") && !dxGrid.startsWith("R+")) {
            double utch = 0.0;
            int nAz, nEl, nDmiles, nDkm, nHotAz, nHotABetter;
            azdist_(const_cast<char*>((m_config.my_grid() + "      ").left(6).toLatin1().constData()),
        ...
        // Strongest signal
        bool convOK;
        int intdbM = dt.report().toInt(&convOK);
...
void MainWindow::on_btn_showAutoLog_clicked()
{
    if (!m_autoLogDlg) {
        m_autoLogDlg = new QDialog(this, Qt::Window);
        m_autoLogDlg->setWindowTitle("Auto CQ / Auto Call Log");
        m_autoLogDlg->resize(700, 400);

        m_autoLogText = new QTextEdit(m_autoLogDlg);
        m_autoLogText->setReadOnly(true);
        m_autoLogText->setLineWrapMode(QTextEdit::NoWrap);
        QFont f("Monospace");
        f.setStyleHint(QFont::TypeWriter);
        f.setPointSize(9);
        m_autoLogText->setFont(f);

        // Header row
        auto col = [](const QString& s, int w) { return s.leftJustified(w); };
        m_autoLogText->setPlainText(
            col("UTC",  8) + "  " + col("Call", 12) + "  " + col("Band", 6) + "  " +
            col("Mode", 6) + "  " + col("Sent", 4) + "  " + col("Rcvd", 4) + "  " +
            col("Grid", 6) + "  Type"
        );

        QPushButton* clearBtn = new QPushButton("Clear", m_autoLogDlg);
        connect(clearBtn, &QPushButton::clicked, [this]() {
            if (m_autoLogText) m_autoLogText->clear();
        });

        QVBoxLayout* vlay = new QVBoxLayout(m_autoLogDlg);
        vlay->addWidget(m_autoLogText);
        QHBoxLayout* hlay = new QHBoxLayout();
        hlay->addStretch();
        hlay->addWidget(clearBtn);
        vlay->addLayout(hlay);
        m_autoLogDlg->setLayout(vlay);
    }
    m_autoLogDlg->show();
    m_autoLogDlg->raise();
...
{
    autoLog(QString("AUTO_HUNT toggled %1  potaWorkedToday=%2")
            .arg(b ? "ON" : "OFF").arg(m_potaWorkedToday.size()));
    if (b) {
        // Auto Hunt requires Auto Call — enable it automatically
        if (!ui->cbAutoCall->isChecked()) {
            ui->cbAutoCall->setChecked(true);
        }
        // Reset the daily worked list on each activation if the date changed
        QString today = QDateTime::currentDateTimeUtc().date().toString("yyyy-MM-dd");
        if (m_potaLastDate != today) {
            m_potaWorkedToday.clear();
            m_potaLastDate = today;
...
{
    autoLog(QString("AUTO_POTA toggled %1").arg(b ? "ON" : "OFF"));
    if (b) {
        // Auto POTA is Auto CQ with a POTA CQ message — exclusive with other auto modes
        ui->cbAutoCall->setChecked(false);
        ui->cbAutoCall->setEnabled(false);
        ui->cbAutoCQ->setChecked(false);
        ui->cbAutoCQ->setEnabled(false);
        ...
        resetAutoSwitch();
        if (!m_autoModeSwitch) clearDX();
        // Auto POTA: always use RR73 so each QSO completes in one fewer transmission.
        m_send_RR73 = true;
        genStdMsgs(m_rpt);
    } else {
        ui->cbAutoCall->setEnabled(true);
        ui->cbAutoCQ->setEnabled(true);
        // Restore normal CQ type when leaving Auto POTA
        m_CQtype = "CQ";
        genCQMsg();
    }
    auto_tx_mode(b);
...
{
    ui->cbAutoCQ->setVisible(visible);
    ui->cbAutoCall->setVisible(visible);
    ui->cbAutoHunt->setVisible(visible);
    ui->cbAutoPOTA->setVisible(visible);
    ui->btn_showOmega->setText(visible ? "Hide Omega" : "Show Omega");
}

void MainWindow::on_btn_clearAutoIgnore_clicked()
{
    m_autoIgnored.clear();
    refreshAutoIgnoredList();
}

void MainWindow::refreshAutoIgnoredList()
{
    QStringList lines;
    auto now = QDateTime::currentDateTimeUtc();
    for (auto it = m_autoIgnored.begin(); it != m_autoIgnored.end(); ++it) {
        if (!it.value().isValid()) {
            lines << it.key() + " — indefinite";
        } else {
            lines << it.key() + " — expires " + it.value().toString("HH:mm:ss") + " UTC";
        }
    }
    lines.sort();
    ui->pte_autoIgnoredList->setPlainText(lines.join('\n'));
}

void MainWindow::on_btn_clearPotaWorked_clicked()
{
    m_potaWorkedToday.clear();
    refreshPotaWorkedList();
}

void MainWindow::refreshPotaWorkedList()
{
    QStringList entries = QStringList(m_potaWorkedToday.values());
    entries.sort();
    ui->pte_potaWorkedList->setPlainText(entries.join('\n'));
}
